...
import com.ktxdevelopment.ddinsurance.model.login.EmailState
import com.ktxdevelopment.ddinsurance.model.login.EmailStateSaver
import com.ktxdevelopment.ddinsurance.theme.MainApplicationTheme
import com.ktxdevelopment.ddinsurance.theme.stronglyDeemphasizedAlpha
import com.ktxdevelopment.ddinsurance.util.supportWideScreen


@Composable
fun WelcomeScreen(onSignInSignUp: (email: String) -> Unit, onSignInAsGuest: () -> Unit, ) {
...
@Composable
private fun Logo(modifier: Modifier = Modifier, lightTheme: Boolean = LocalContentColor.current.luminance() < 0.5f ) {

    val assetId = if (lightTheme) { R.drawable.ic_launcher } else { R.drawable.ic_launcher }
    Image(
...
private fun SignInCreateAccount(
    ...
    Column(modifier = modifier, horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        Image(painter = painterResource(id = R.drawable.ic_google), contentDescription = "google",
            modifier = Modifier.padding(start = 30.dp, end  = 30.dp, bottom = 30.dp).clickable(true, onClick = { onSignInAsGuest() }))
        Image(painter = painterResource(id = R.drawable.ic_facebook), contentDescription = "facebook",
            modifier = Modifier.padding(start = 30.dp, end = 30.dp).clickable(true, onClick = { onSignInAsGuest() }))
    }
}
